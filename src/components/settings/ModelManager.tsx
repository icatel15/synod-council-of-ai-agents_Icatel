...
export default function ModelManager() {
  ...
  const [selectedOrModel, setSelectedOrModel] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleAddModel = () => {
    ...
    if (selectedProvider === 'openrouter') {
      if (customModelId.trim()) {
        const rawId = customModelId.trim();
        modelId = orWebSearch && !rawId.endsWith(':online') ? `${rawId}:online` : rawId;
        displayName = orWebSearch ? `${rawId} (Web)` : rawId;
      } else if (selectedOrModel) {
        modelId = selectedOrModel.id;
        displayName = selectedOrModel.name;
      } else {
        return;
      }
    } else {
    ...
    <div className="space-y-6">
        ...
        {showAddModel ? (
          <div className="p-3 rounded-[var(--radius-md)] border border-dashed border-[var(--color-border-secondary)] bg-[var(--color-bg-secondary)] space-y-2">
            ...
            {selectedProvider === 'openrouter' ? (
              <>
                ...
                <OpenRouterModelSearch
                  onSelect={(model) => {
                    // Auto-add to council immediately on selection
                ...
                <div>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value) setSelectedOrModel(null);
                    }}
            ...
            <div className="flex gap-2">
              ...
              >
                {selectedProvider === 'openrouter' ? 'Add Custom' : 'Add'}
              </Button>
              <Button
                size="sm"
                variant="ghost"
      ...
      <div>
        ...
        <div className="space-y-2">
          <select
            ...
            onChange={(e) => {
              ...
              if (firstModel) {
                ...
                // Set provider immediately; model will be set via search
                updateSettings({ masterModel: { provider, model: '' } });
              }
            }}
            className="w-full px-3 py-1.5 text-sm bg-[var(--color-bg-input)] border border-[var(--color-border-primary)] rounded-[var(--radius-sm)] text-[var(--color-text-primary)]"
          >
          ...
          {settings.masterModel.provider === 'openrouter' ? (
            <div>
              {settings.masterModel.model && (
                <p className="text-xs text-[var(--color-text-secondary)] mb-1">
                  Selected: <span className="font-medium">{settings.masterModel.model}</span>
                </p>
              )}
              <OpenRouterModelSearch
                onSelect={(model) => handleMasterModelChange('openrouter', model.id)}
              />
            </div>
          ) : (
            <select
